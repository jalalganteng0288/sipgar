    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-8 rounded-lg shadow-lg">
                <form action="{{ route('admin.house-types.update', $houseType->id) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                :value="old('name', $houseType->name)" required />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <x-text-input id="building_area" name="building_area" type="number"
                                class="mt-1 block w-full" :value="old('building_area', $houseType->building_area)" required />
                        </div>
                        <div>
                            ...
                            <select id="status" name="status"
                                ...
                                <option value="Proses Bank"
                                    {{ old('status', $houseType->status) == 'Proses Bank' ? 'selected' : '' }}>Proses
                                    Bank</option>
                        ...
                        <div>
                            ...
                            <x-text-input id="total_units" name="total_units" type="number" class="mt-1 block w-full"
                                :value="old('total_units', $houseType->total_units)" required />
                        </div>
                        <div class="mt-4">
                            ...
                            <x-text-input id="ready_stock" name="ready_stock" type="number" class="mt-1 block w-full"
                                value="{{ old('ready_stock', $houseType->ready_stock) }}" required />
                            <x-input-error :messages="$errors->get('ready_stock')" class="mt-2" />
                        </div>
                        <div class="md:col-span-2">
                            <x-input-label for="description" :value="__('Deskripsi Singkat Tipe Rumah')" />
                            <textarea id="description" name="description" rows="3"
                                class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $houseType->description) }}</textarea>
                        </div>
                        ...
                        <div class="md:col-span-1">
                            ...
                            @if ($houseType->image)
                                <img src="{{ asset('storage/' . $houseType->image) }}" alt="Foto Tipe saat ini"
                                    class="w-48 h-auto mt-2 rounded-md border p-1">
                            @endif
                        </div>
                        <div class="md:col-span-1">
                            ...
                            @if ($houseType->floor_plan)
                                <img src="{{ asset('storage/' . $houseType->floor_plan) }}" alt="Denah saat ini"
                                    class="w-48 h-auto mt-2 rounded-md border p-1">
                            @endif
                        </div>
                        {{-- AKHIR PERBAIKAN --}}
