    ...
    <section class="hero-section text-white py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="mt-4 text-lg max-w-3xl opacity-95">
                <span class="font-semibold">ID Lokasi: {{ $project->id }}</span>
                <br>
                {{ $project->address }}, {{ optional($project->village)->name }},
            ...
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-8 px-8" aria-label="Tabs">
                    <button @click="activeTab = 'tipe'"
                        :class="activeTab === 'tipe' ? 'border-indigo-500 text-indigo-600' :
                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">Tipe Rumah</button>
                    <button @click="activeTab = 'detail'"
                        :class="activeTab === 'detail' ? 'border-indigo-500 text-indigo-600' :
                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">Detail Proyek</button>
                    <button @click="activeTab = 'galeri'"
                        :class="activeTab === 'galeri' ? 'border-indigo-500 text-indigo-600' :
                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">Galeri</button>
                    <button @click="activeTab = 'siteplan'"
                        :class="activeTab === 'siteplan' ? 'border-indigo-500 text-indigo-600' :
                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">Siteplan</button>
                </nav>
            </div>
            ...
            <div class="p-6 md:p-8">
                                ...
                                <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        ...
                                        <ul class="text-gray-600 space-y-1 text-sm">
                                            <li><strong>Harga:</strong> <span class="text-green-600 font-bold">Rp
                                                    {{ number_format($type->price, 0, ',', '.') }}</span></li>
                                    ...
                                    <div>
                                        ...
                                        <img src="{{ $type->floor_plan ? asset('storage/' . $type->floor_plan) : 'https://via.placeholder.com/400x250.png?text=Denah' }}"
                                            alt="Denah {{ $type->name }}"
                                            class="rounded-lg border w-full object-cover aspect-video">
                                    </div>
                                </div>
                            </div>
                        @empty
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <p class="text-gray-600 mt-1 prose max-w-none">
                ...
                <div x-show="activeTab === 'galeri'" x-cloak>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        @if ($project->image)
                            <a href="{{ asset('storage/' . $project->image) }}" target="_blank"><img
                                    src="{{ asset('storage/' . $project->image) }}" alt="Foto Utama"
                                    class="aspect-video w-full object-cover rounded-lg shadow hover:shadow-xl transition-shadow"></a>
                        @endif
                        @forelse($project->images as $image)
                            <a href="{{ asset('storage/' . $image->path) }}" target="_blank"><img
                                    src="{{ asset('storage/' . $image->path) }}" alt="Foto Galeri"
                                    class="aspect-video w-full object-cover rounded-lg shadow hover:shadow-xl transition-shadow"></a>
                        @empty
                            @if (!$project->image)
                ...
                <div x-show="activeTab === 'siteplan'" x-cloak>
                    ...
                    @if ($project->site_plan)
                        <img src="{{ asset('storage/' . $project->site_plan) }}" alt="Siteplan {{ $project->name }}"
                            class="rounded-lg border w-full">
                    @else
                        <div class="bg-gray-200 rounded-lg flex items-center justify-center min-h-[400px]">
                            <p class="text-gray-500">Siteplan Digital Belum Tersedia.</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const observer = new MutationObserver((mutations) => {
                for (const mutation of mutations) {
                    if (mutation.attributeName === 'style' && mutation.target.style.display !== 'none' &&
                        mutation.target.getAttribute('x-show') === 'activeTab === \'detail\'') {
                        initMap();
                        break;
                    }
                }
            ...
            if (mapContainerParent) {
                observer.observe(mapContainerParent, {
                    attributes: true
                });
            }
        });
